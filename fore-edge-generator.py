...
def convert_color_profile(img, target_mode):
    ...
    if img.mode != target_mode:
        print(f"Converting image from {img.mode} to {target_mode}...")
...
def process_book():
    print("\n\n--- Antithesis Press' Quick and Dirty Fore-Edge Generator ---")
    print("\n------------------Release: 2026/06/02------------------------")
    print("\n**This script is provided without warranty either expressed or implied.**\n**The author assumes no liability for any damage howsoever caused by using this script.**\n")
    print("Free for commercial and individual use. See Github readme for licence details.\n")
    print("\n *You will require an input PDF file and PNG image file(s) for the top, bottom and fore-edge sides*")
    ...
    try:
        ...
        print("Invalid weight. Defaulting to standard 50 lb.\n")
        weight_lb = 50.0
    ...
    print("Note: Take account of the safety margin in this calculation, the bleed size will be added on (in a moment!)")
    safety_input = input("\nEnter the thickness of the desired border from PDF trim box to safety boundary (e.g. '0.177 for inches, or '4.5mm'): ").strip().lower()
    ...
    if safety_input.endswith('mm'):
        try:
            ...
            print("Invalid metric input. Defaulting to 4.5mm.")
        ...
        try:
            ...
            print("Invalid input. Defaulting to 0.177 inches.")
            BORDER_PTS = 0.177 * 72

    # Get bleed value
    # ...and check they make sense / use defaults again.
    bleed_value = input("\nEnter bleed size (e.g. '0.125' for inches, or '3.175mm'): ").strip().lower()
    if bleed_value.endswith('mm'):
        try:
            bleed_mm = float(bleed_value.replace('mm', '').strip())
            bleed_pts = bleed_mm * (72 / 25.4)
        except ValueError:
            print("Invalid metric input. Defaulting to 3.175mm.")
        ...
        try:
            ...
            print("Invalid input. Defaulting to 0.125 inches.")
            bleed_pts = 0.125 * 72
    ...
    print("\nScaling and cropping images to fit physical book proportions...")
    """
    ...
    print("Applying edges up to the safety boundary...\n")
    for i in range(sheets):
        ...
        # Create Proof Images
        """
        Each individual slice is reassembled back into a single image.
        """
        # Recto
        ...
        proof_mid_v.paste(verso_mid_strip, (0,i))
        ...
        if recto_idx < page_count:
            ...
            #use trim box rather than page dimensions for better alignment between pages (we hope)
            page.insert_image(get_aligned_rect(r_trim, "top", BORDER_PTS), stream=img_to_bytes(top_strip), keep_proportion=False)
            page.insert_image(get_aligned_rect(r_trim, "bottom", BORDER_PTS), stream=img_to_bytes(bot_strip), keep_proportion=False)
            page.insert_image(get_aligned_rect(r_trim, "fore", BORDER_PTS), stream=img_to_bytes(mid_strip, rotate=True), keep_proportion=False)
        ...
        if verso_idx < page_count:
            ...
            page.insert_image(get_aligned_rect(v_trim, "top", BORDER_PTS), stream=img_to_bytes(top_strip, mirror=True), keep_proportion=False)
            page.insert_image(get_aligned_rect(v_trim, "bottom", BORDER_PTS), stream=img_to_bytes(bot_strip, mirror=True), keep_proportion=False)
            page.insert_image(verso_fore, stream=img_to_bytes(mid_strip, rotate=True), keep_proportion=False)
